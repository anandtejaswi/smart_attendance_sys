...
class DataManager:
    ...
    def insert_user(
        ...
        try:
            ...
            # Parameterized queries to prevent SQL injection (Phase 7
            # prerequisite)
        ...
        try:
            ...
            for row in rows:
                if self.db_manager.db_type == 'sqlite':
                    ...
                    encoding_bytes = bytes(row[1])

                # Deserialize back into a numpy float64 array
                encoding_array = np.frombuffer(
                    encoding_bytes, dtype=np.float64)
    ...
    def export_attendance_analytics(self, expected_sessions=1, export_dir="."):
        ...
        try:
            ...
            # Extract main attendance string logic matching users with their
            # timestamps
            query = '''
                SELECT
                    a.log_id,
            ...
            analytics_df = df.groupby(['user_id', 'name', 'dept']).size(
            ).reset_index(name='sessions_attended')

            # Avoid division by zero
            safe_sessions = expected_sessions if expected_sessions > 0 else 1
            analytics_df['attendance_percent'] = (
                analytics_df['sessions_attended'] / safe_sessions) * 100
            ...
            # engine requires openpyxl installed which is pip standard for
            # pandas excel
            ...
            print(
                f"Successfully exported comprehensive attendance reports to {export_dir}")
            return True
