...
class TestIntegrationPipeline(unittest.TestCase):
    ...
    def test_capture_to_log_pipeline(self, mock_detect, mock_encode):
        ...
        # Insert target user
        self.dm.insert_user(target_uid, "Pipeline Test", "QA", target_encoding, "hash", "User")
        ...
        for frame_idx in range(1, 5):
            ...
            if bboxes:
                ...
                for uid, saved_enc in known_encodings.items():
                    dist, is_match = self.engine.compare_encoding(current_encoding, saved_enc)
                    if is_match:
                        matched_uid = uid
                        break
                ...
                if matched_uid:
                    if self.engine.check_stability(matched_uid):
                        # 5. Database Logging
                        success = self.dm.log_attendance(matched_uid, confidence=(1.0 - dist))
                        if success:
                            logs_written += 1
            else:
        ...
        # Assertions
        self.assertEqual(logs_written, 1, "Should have precisely 1 attendance log from the 4 frames")
        
        # Verify database insertion
        conn = self.dm.db_manager.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM Attendance_Logs WHERE user_id = ?", (target_uid,))
        records = cursor.fetchall()
        self.dm.db_manager.close_connection(conn)
        
        self.assertEqual(len(records), 1, "Exactly one log should exist in the database")
